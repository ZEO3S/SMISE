...
export default function Slider({ minValue, maxValue, min, max, step = 1, setMinValue, setMaxValue, onChange }: Props) {
    ...
    <div className='flex items-center relative w-full h-16 mt-3' ref={sliderRef}>
      ...
      >
        ...
        >
          ...
          {minValue !== max && <Text content={minValue === min ? '신입' : `${minValue}년`} />}
        </div>
        <div
          className='absolute w-10 cursor-pointer'
          style={{
